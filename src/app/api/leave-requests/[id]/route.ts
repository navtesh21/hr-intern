  ...
  try {
    ...
    if (status === "APPROVED") {
      ...
      const availableBalance =
        leaveRequest.employee.leaveBalance - usedLeaveDays;
        ...
        return NextResponse.json<ApiResponse>(
          {
            success: false,
            error:
              "Cannot approve: Leave request overlaps with existing approved leave",
            data: {
    ...
    if (status === "APPROVED") {
      // Transaction: update leave request and decrement employee leave balance
      ...
      updatedLeaveRequest = result[0];
      console.log("Leave request approved:", updatedLeaveRequest);
    } else {
      // Only update leave request (for REJECTED)
    ...
    leaveDays = calculateLeaveDays(
      new Date(updatedLeaveRequest.startDate),
      new Date(updatedLeaveRequest.endDate)
    );
